...
const LocationSelector = ({
  ...
  const [statesData, setStatesData] = useState<StateProps[]>([])
  const [isLoading, setIsLoading] = useState(true)

  useEffect(() => {
    const loadData = async () => {
      try {
        const [countriesModule, statesModule] = await Promise.all([
          import('@/data/countries.json'),
          import('@/data/states.json')
        ])

        setCountriesData(countriesModule.default as CountryProps[])
        setStatesData(statesModule.default as StateProps[])
      } catch (error) {
  ...
  const [selectedCountry, setSelectedCountry] = useState<CountryProps | null>(() => {
    if (defaultCountry && countriesData.length > 0) {
      return countriesData.find(
        (country) => country.iso2.toLowerCase() === defaultCountry.toLowerCase()
      ) || null
    }
    return null
  })
  ...
  useEffect(() => {
    if (defaultCountry && countriesData.length > 0 && !selectedCountry) {
      const country = countriesData.find(
        (country) => country.iso2.toLowerCase() === defaultCountry.toLowerCase()
      )
      setSelectedCountry(country || null)
    }
  }, [defaultCountry, countriesData, selectedCountry])
  ...
  return (
    <div className="flex gap-4">
      ...
      {availableStates.length > 0 && enableState && (
